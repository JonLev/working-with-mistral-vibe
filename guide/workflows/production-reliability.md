...
description: "Escalation design, circuit breakers, structured error propagation, graceful degradation, human handoff, and source conflict resolution for systems built around an agentic CLI"
...
Structure and pedagogy adapted from the source guide (CC BY-SA 4.0, see [`NOTICE.md`](../../NOTICE.md)). The patterns are language-agnostic; the Vibe mechanics referenced in "Running the checks with Vibe" cite the mechanics oracle, [`verified-mechanics.md`](../../docs/mechanics/verified-mechanics.md), inline as `(PART-XXX)`. Backend tags `[stable]` / `[unified-harness]` mark mechanics that depend on the session backend.

## TL;DR

```text
1. Escalate on programmatic signals (explicit request, policy gap, retry
   budget exhausted) — never on model confidence scores
2. Put a circuit breaker around every failing dependency
3. Propagate structured errors (is_retryable, error_category,
   alternative_approach) — not exception strings
4. Return partial results with coverage annotations, not nothing
5. Hand off structured payloads with a recommended next action
6. Resolve source conflicts by date when possible; surface the rest
```

**Read if** you operate agent-powered pipelines that touch production systems or customers and need them to fail predictably. **Skip if** you only run interactive sessions where you are the escalation path.

## The problem

Agent-powered systems fail in ways that differ from traditional software. The model may produce syntactically valid output that is semantically wrong, may refuse to continue when it cannot make progress, or may produce partial results when source data is incomplete. The patterns below address each of these failure modes in production. They are written around an agentic CLI, but none of them are specific to one: the CLI provides the loop, budgets, and gates; escalation policy, circuit state, and handoff payloads are your code around it.
...
**1. Explicit user request:** the customer explicitly asks for a human. This is non-negotiable and takes priority over every other signal, including frustration detection. "I want to speak to a human" is a different signal from an angry tone, and it requires immediate handoff regardless of what the agent could have done next.
...
Conflating them is a common mistake with real consequences: routing frustrated users to humans when they just wanted a better answer, or continuing to try harder when the user has already decided they want a human.
...
class HandoffPayload:
    ...
    root_cause: str               # why the agent could not resolve this
...
charge processed on May 18. The agent confirmed account details and found the
charge ($49.00) but hit a policy gap: refund approval for charges older than 7
days requires manual authorization. Two escalation attempts were made; the
customer then explicitly requested a human agent.
...
## Running the checks with Vibe

The patterns above are orchestrator code. Vibe's documented surface supplies the primitives they sit on — budgets, deterministic capture, in-session guards, and scheduled check-ins — and nothing else: there is no first-party CI integration, webhook system, or escalation API in the verified surface, so the escalation logic is always yours. The verified mechanics, all `[stable]` unless noted:

| Reliability need | Vibe mechanic | Citation |
|---|---|---|
| Retry / effort budget | `--max-turns N` caps assistant turns; `--max-price DOLLARS` and `--max-tokens N` interrupt the session when exceeded (all `-p`-mode only) | PART-CLI |
| Deterministic result capture | `--output json` emits all messages at session end; `--output streaming` emits newline-delimited JSON per message | PART-CLI |
| Human-free runs | In `-p` mode every approval- or input-requesting callback is auto-denied, and `ask_user_question` is force-disabled — a headless pipeline cannot stall on a prompt; `--auto-approve`/`--yolo` or an agent/permission config that allows the call is required for anything gated | PART-CLI; PART-TRUST §3.4 |
| Non-interactive trust | `--trust` trusts the working directory for this invocation only; `trusted_folders.toml` is not modified | PART-TRUST §3.3 |
| Pre-flight guard on tool calls | `hooks.toml` `pre_tool` hook: stdout JSON `{"decision": "deny", "reason": ...}` blocks the call; `strict = true` makes hook failure deny instead of warn | PART-HOOKS §1-3 |
| Bounded auto-retry per turn | `post_agent` deny injects the reason as a retry message, max 3 retries per user turn | PART-HOOKS §3.7 |
| Scheduled in-session check-ins | `/loop <interval> <prompt>` — min 30 s interval, max 50 loops per session, fires only when the session is idle, survives resume | PART-SESSIONS §5 `[both]` |

### Example: a fail-closed guard hook

```toml
# <project>/.vibe/hooks.toml
[[hooks]]
name = "guard-destructive-bash"
type = "pre_tool"
match = "bash"
command = "python ./.vibe/hooks/guard_bash.py"
strict = true
description = "Deny destructive shell commands in the reliability pipeline."
```

The guard reads the tool invocation JSON from stdin and denies with a structured reason (PART-HOOKS §3.2-3.3). Note the `strict = true`: by default hooks **fail open** — a crashing or timing-out guard only warns and lets the call proceed — which is the wrong default for a reliability gate (PART-HOOKS §3). Hook timeout defaults to 60 seconds (PART-HOOKS §3.5).

### Example: a bounded, machine-readable check run

```text
vibe --trust -p "Run the release checklist in scripts/release-check.sh and report each failing step." --max-turns 20 --max-price 2.00 --output json
```

Parse the JSON entries, apply the escalation function, and exit non-zero or page a human from your wrapper — the CLI run itself is deterministic in shape (`--output json`, PART-CLI) and bounded in cost (`--max-turns`, `--max-price`, PART-CLI). Because approval-requiring calls are auto-denied in `-p` mode unless allowed by config, a read-only check run cannot drift into writes (PART-CLI; PART-TRUST §3.4).

### Example: scheduled in-session check-ins

`/loop 30m Check open PRs for failing CI and summarize any release blockers.` schedules a recurring prompt with an ID, listed via `/loop list` and cancellable via `/loop cancel <id|all>` (PART-SESSIONS §5). Constraints that matter for reliability design: the interval floor is 30 seconds, the session caps at 50 loops, prompts cannot start with `/`, and firing is idle-only — a loop never interrupts a running turn, so a stuck turn means no check-in, not a queued one. Loops persist in the session's `meta.json` and are restored on resume (PART-SESSIONS §5).
...
**Trusting a fail-open guard.** A `pre_tool` hook that crashes, times out, or prints malformed JSON lets the gated call proceed with a warning unless `strict = true` (PART-HOOKS §3). If a hook is your safety boundary, it must fail closed.

## Known gaps

- **Live-run note (2026-09-24, vibe 2.25.7).** Fail-open vs `strict` guard semantics live-verified: a crashing `pre_tool` guard (non-zero exit) lets the gated call proceed (effect `completed`), while the same guard with `strict = true` denies it (error notice `Denied tool 'bash' (strict)`, effect `skipped`), matching PART-HOOKS §3. One caveat: the fail-open *warning* is UI-only — it does not appear in `-p` text output or in `--output json` entries (only `Running hooks` / `Hooks completed` info notices), so a headless wrapper parsing the JSON cannot detect that a fail-open guard crashed. If a crashed guard must be observable from a pipeline, make the guard `strict` or check its side effects yourself. Also, the guard examples invoke `python`; on hosts where only `python3` exists the hook command spawn-fails, which routes to the same failure path (warn / strict-deny).
- **No native escalation or handoff primitives.** Vibe's documented surface has no ticketing, webhook, or escalation API. Everything in "Running the checks with Vibe" is CLI invocation plus your wrapper; the handoff payload must be written to a file or external system by your code.
- **Hooks fail open by default.** Every guard built on `hooks.toml` warns-and-proceeds on hook failure unless `strict = true` (tool hooks only) (PART-HOOKS §3). There is no global fail-closed switch.
- **`/loop` is in-session and interactive.** It is not a cron scheduler and not a CI mechanic; scheduled loops live inside one session (restored on resume), the session caps at 50, and the unified-harness implementation is disabled when headless (PART-SESSIONS §5). Cross-process scheduling is external infrastructure.
- **No mid-run human in programmatic mode.** `-p` runs auto-deny every approval and user-input callback and force-disable `ask_user_question` (PART-TRUST §3.4). Escalation "to a human" from a headless run means ending the run and escalating from the wrapper — the run itself cannot pause for judgment.
- **No persistent task store.** Handoff state, retry counters, and circuit state have no built-in persistence in the verified surface; keep them in files or your datastore, not in session context.

## See also

- [Agent Harness Engineering](../core/agent-harness.md) — failure modes and verification boundaries of the runtime the CLI gives you
- [Loop & Graph Engineering](../core/loop-graph-engineering.md) — stopping rules and feedback loops, the theory behind the budgets above
- [Hooks Events Reference](../core/hooks-events-reference.md) — the full `hooks.toml` wire protocol behind the guard example
- [Settings Reference](../core/settings-reference.md) — `--max-turns`/`--max-price`/`--max-tokens` and the config keys that bound a session
- [Tools Reference](../core/tools-reference.md) — the permission chain that decides which tool calls need approval
- [Best-of-N: Generate, Select, Verify](./best-of-n.md) — verification discipline when a run produces candidate results
