...
description: Compare approaches to the same problem - three to five alternatives with trade-offs, a comparison table against standard criteria, and a recommendation based on context. Use when the user asks for alternatives, options, or a comparison for a design decision; not when they want a single implementation.
---
...
Text after `/learn-alternatives` arrives as extra instructions: treat it as
the topic, with any depth flags.
...
Vibe: ## Problem: Managing Form State in React
