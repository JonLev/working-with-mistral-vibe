#!/usr/bin/env python3
"""Dependency-free state engine for the learning-path progress skill."""
...
STATE_VERSION = 1
HOME_STATE_RELATIVE_PATH = Path("learning/progress.json")
PROJECT_STATE_RELATIVE_PATH = Path(".vibe/learning/progress.json")
...
def vibe_home() -> Path:
    """Resolve the VIBE_HOME tree the way the CLI does: $VIBE_HOME or ~/.vibe."""
    return Path(os.getenv("VIBE_HOME", str(Path.home() / ".vibe"))).expanduser().resolve()


def home_state_path(home: Path) -> Path:
    """Cross-project state inside the VIBE_HOME tree."""
    return home / HOME_STATE_RELATIVE_PATH


def project_state_path(project: Path) -> Path:
    """Per-project state inside the project's .vibe tree."""
    return project / PROJECT_STATE_RELATIVE_PATH
...
def load_state(state_file: Path, path_data: Dict[str, Any]) -> Dict[str, Any]:
...
def save_state(state_file: Path, state: Dict[str, Any], path_data: Dict[str, Any]) -> None:
    """Atomically replace the state file after validating the complete payload."""
    _validate_state(state, path_data)
    state_file.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(prefix=".progress-", suffix=".tmp", dir=str(state_file.parent))
...
def create_profile(state_file: Path, path_data: Dict[str, Any], track: str) -> Dict[str, Any]:
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--home", type=Path, default=None, help="VIBE_HOME tree for cross-project state (default: $VIBE_HOME or ~/.vibe)")
    parser.add_argument("--project", type=Path, default=None, help="Project directory whose .vibe/learning/ tree stores per-project state")
...
def _state_target(args: argparse.Namespace) -> Path:
    if args.project is not None and args.home is not None:
        raise ProgressError("Pass either --home or --project, not both")
    if args.project is not None:
        return project_state_path(args.project)
    if args.home is not None:
        return home_state_path(args.home)
    return home_state_path(vibe_home())
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        state_file = _state_target(args)
        if args.command == "init":
            print(json.dumps(create_profile(state_file, path_data, args.track), indent=2, sort_keys=True))
            return 0
        state = load_state(state_file, path_data)
        if args.command == "complete":
            complete_module(state, path_data, args.module_id, args.evidence, args.completed_on)
            save_state(state_file, state, path_data)
