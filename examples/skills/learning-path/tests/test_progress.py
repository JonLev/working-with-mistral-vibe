...
class ProgressTests(unittest.TestCase):
    ...
    def test_creates_a_new_beginner_profile_in_the_home_tree(self) -> None:
        state_file = progress.home_state_path(self.home)

        state = progress.create_profile(state_file, self.path, "Beginner")

        self.assertEqual(state["track"], "Beginner")
        self.assertTrue(state_file.is_file())
        self.assertEqual(state_file, self.home / "learning" / "progress.json")
        self.assertEqual(json.loads(state_file.read_text(encoding="utf-8"))["modules"], {})

    def test_project_state_lands_in_the_project_vibe_tree(self) -> None:
        state_file = progress.project_state_path(self.project)

        progress.create_profile(state_file, self.path, "Practitioner")

        self.assertEqual(state_file, self.project / ".vibe" / "learning" / "progress.json")
        self.assertTrue(state_file.is_file())
    ...
    def test_concurrent_profile_creation_allows_exactly_one_winner(self) -> None:
        ...
        def create_profile(index: int) -> str:
            start.wait()
            try:
                return progress.create_profile(state_file, self.path, "Beginner")["track"]
        ...
        self.assertEqual(progress.load_state(state_file, self.path)["track"], "Beginner")

    def test_atomic_save_leaves_complete_json_without_a_temporary_file(self) -> None:
        state_file = progress.home_state_path(self.home)
        state = progress.new_state("Practitioner")
        progress.save_state(state_file, state, self.path)
    ...
    def test_atomic_save_keeps_existing_bytes_when_replace_fails(self) -> None:
        ...
        progress.save_state(state_file, original, self.path)
        ...
        with mock.patch.object(progress.os, "replace", side_effect=OSError("injected failure")):
            with self.assertRaisesRegex(progress.ProgressError, "Could not save"):
                progress.save_state(state_file, progress.new_state("Production"), self.path)
    ...
    def test_next_module_selects_the_first_available_module_for_the_track(self) -> None:
        state = progress.new_state("Practitioner")
        progress.complete_module(state, self.path, "module-01", "Installed the Vibe CLI and ran --help")
    ...
    def test_review_schedule_uses_the_required_intervals(self) -> None:
        ...
        progress.complete_module(
            ...
            self.path,
            "module-01",
            "Installed the Vibe CLI and recorded the version",
    ...
    def test_corrupt_state_fails_closed(self) -> None:
        ...
        with self.assertRaisesRegex(progress.ProgressError, "corrupt"):
            progress.load_state(state_file, self.path)
        ...
        for command in commands:
            with self.subTest(command=command):
                ...
                with contextlib.redirect_stderr(io.StringIO()):
                    self.assertEqual(progress.main(["--home", str(self.home)] + command), 2)

    def test_default_state_target_follows_the_vibe_home_environment(self) -> None:
        with mock.patch.dict(os.environ, {"VIBE_HOME": str(self.home)}):
            args = argparse.Namespace(home=None, project=None)
            expected = self.home.resolve() / "learning" / "progress.json"
            self.assertEqual(progress._state_target(args), expected)

    def test_cli_rejects_home_and_project_at_once(self) -> None:
        with contextlib.redirect_stderr(io.StringIO()):
            self.assertEqual(
                progress.main(["--home", str(self.home), "--project", str(self.project), "status"]),
                2,
            )
