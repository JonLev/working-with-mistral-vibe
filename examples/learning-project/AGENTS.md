...
The [evidence-reviewer subagent](.vibe/agents/evidence-reviewer.toml) reviews evidence but cannot edit files or run commands: its profile enables only `read_file` and `grep` and denies `write_file` outright. The [verify-release skill](.vibe/skills/verify-release/SKILL.md) may run local checks, but it must stop before publication. The [release-guard hook](.vibe/hooks.toml) is a narrow release-action gate and does not replace the [hooks and events reference](../../guide/core/hooks-events-reference.md).
