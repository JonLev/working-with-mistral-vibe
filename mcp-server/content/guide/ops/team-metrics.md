...
description: "How to measure and pilot a tech-product team practicing agentic coding. DORA, SPACE, the metrics that matter when agents write much of your code, and the measurement surfaces Vibe actually gives you."
tags: [guide, metrics, dora, space, team, observability, ai-augmented]
---

# Team Metrics for AI-Augmented Engineering

> Velocity is easy to measure and easy to misread. Agentic coding raises the bar for what "moving fast" even means.
The epigraph is carried from the source guide.
> **Verified against vibe 2.25.8 on 2026-09-24.** Documented surface: release 2.25.8.
>
> Mechanics — commands, flags, config keys, file paths, payload fields — cite the mechanics
> oracle, [`verified-mechanics.md`](../../docs/mechanics/verified-mechanics.md), as
> `(PART-XXX)`. Claims executed against the installed CLI say *live-verified on 2.25.7* and
> cite [`docs/mechanics/live-checks.md`](../../docs/mechanics/live-checks.md);
> the rest is source-verified at release 2.25.8. Backend tags `[stable]` /
> `[unified-harness]` / `[both]` mark backend-dependent mechanics. The framework content
> (DORA, SPACE, external studies) is adapted from the source guide and kept as its dated
> external evidence, not as anything this guide's oracle verified.
The Vibe measurement surfaces are catalogued in the tooling section near the end.
> **TL;DR.** Keep DORA as the delivery-system foundation, SPACE for the people inside it,
> and add the AI-specific and agentic metrics the standard frameworks don't cover — while
> distrusting every activity metric that goes up just because agents write more code. For
> Vibe specifically, the verified measurement surfaces are: the session store
> (`$VIBE_HOME/logs/session/`, `meta.json` + `messages.jsonl`), the `/status` command,
> headless runs with `--output json` in CI, OTel export for org-level pipelines, and
> `--max-price` / `--max-tokens` as budget instrumentation. No third-party dashboard
> computes the AI-specific metrics for you; the session store is the raw material.

**Read if** you fund, lead, or operate an engineering team adopting agentic coding and need
a measurement stack that survives it. **Skip if** you want the Vibe capture mechanics
themselves — [Session Observability](./observability.md) is the how-to page; this one is
about which numbers to compute and which to distrust.
...
6. [Measure the Shared Verification Queue](#measure-the-shared-verification-queue)
7. [Agentic Metrics: What DORA Doesn't Measure](#agentic-metrics-what-dora-doesnt-measure)
8. [Product Metrics (the often-missing layer)](#product-metrics-the-often-missing-layer)
9. [By Team Size](#by-team-size)
10. [Vanity Metrics to Drop](#vanity-metrics-to-drop)
11. [The 4-Question Test](#the-4-question-test)
12. [Tooling](#tooling)
13. [Probabilistic Delivery Forecasting](#probabilistic-delivery-forecasting)
14. [Implementation Roadmap](#implementation-roadmap)
15. [Reporting Delivery Capacity to a Skeptical Board](#reporting-delivery-capacity-to-a-skeptical-board)
...
AI-assisted development changes delivery speed fast enough to break most existing benchmarks. A team shipping 2 features per sprint in 2022 might now ship 6, with agents generating 70-90% of the code. That looks like a win on every traditional scorecard, and it might genuinely be one — or the velocity is hiding shallow reviews, skill atrophy, and a growing pile of AI-generated technical debt that nobody fully understands.
...
This page gives engineering managers, tech leads, and CTOs a practical measurement stack: the DORA foundation, human factors via SPACE, then the AI-specific and agentic signals the standard frameworks don't cover. The capture mechanics for Vibe live on [Session Observability](./observability.md); this page decides what to compute.
...
**Common pitfall**: Lead time measures calendar time, not active work time. A PR that sits in review for 3 days has 3 days of lead time even if the actual coding took 20 minutes. If your lead time is long, check where it is accumulating: review queues, staging environments, or deployment pipelines.
...
Each DORA metric reacts differently when agents enter the development workflow. Understanding those effects helps you set the right targets and spot the right warning signs.

### Deployment Frequency under agents

Agents accelerate feature development, so your deployment cadence should increase, provided your pipeline can keep up. If deployment frequency stays flat after widespread adoption, the bottleneck is downstream: staging environments, manual QA gates, or review throughput — not coding speed. Agents give you more PRs to merge; they don't automatically improve the rest of the pipeline.

One large AI vendor's engineering team published the shape of that downstream bottleneck (September 2026; the source guide carries a full evaluation of the post): tests across the codebase grew 10x with only a nominal number of engineers added, which translated into a 25x increase in CI jobs over six months. This is a single company's trajectory, not a published counting methodology or a benchmark, and the three figures don't multiply out because not every test runs on every PR. The dimensioning heuristic still travels: assume your CI architecture will be at 25x its current load within two quarters, and design for 10-20x the perceived scale rather than the observed scale.
...
### Change Lead Time under agents

Agents cut coding time but have limited effect on the non-coding segments of lead time. PR review, staging validation, context-switching delays, and deployment windows are largely unchanged by AI assistance. If your lead time isn't improving alongside adoption, the constraint is in review velocity or pipeline automation, not coding. Map your lead time stages explicitly (code time, review wait, staging wait, deploy window) to know where the leverage is.

### Change Failure Rate under agents

This is the metric most at risk when AI adoption outpaces review discipline. Agents generate syntactically correct, structurally plausible code that can still have subtle behavioral errors. Teams that treat agent-generated PRs as "lower risk" and rubber-stamp reviews tend to see CFR creep up over 6-12 months. The failure mode is gradual: each individual PR looks fine, but the cumulative effect of reduced scrutiny shows up in production.

Track CFR separately for agent-generated code versus manually written code — but read [AI-Specific Metrics](#ai-specific-metrics) first, because in Vibe that split is a convention you build, not a feature you enable. If agent-generated CFR is materially higher, your review process needs reinforcement, not your AI tooling.

### Failed Deployment Recovery Time under agents

Agent-assisted diagnosis can reduce recovery time when the model has access to deployment metadata, error logs, traces, and codebase context. The claim must be measured rather than inferred from a demo. Track the recovery-time distribution, false diagnoses, human intervention time, and whether the failed deployment was actually the cause. Instrument first, then compare the same service against its pre-adoption baseline.

### Deployment Rework Rate under agents

Generation speed can hide corrective work. Track whether agent-assisted changes produce more hotfix, rollback, and follow-up deployments, then attach token cost and human review time to that rework (the cost surfaces are in [Session Observability](./observability.md)). A higher deployment frequency with a rising rework rate is not a throughput improvement.
...
Critical note: Activity is the easiest dimension to measure and the easiest to game. High commit count, high PR volume, high deployment frequency can all coexist with low actual value delivered. Agents inflate every activity metric by construction. Activity metrics are inputs, not outcomes.
...
Teams can hit "High" on DORA deployment frequency while simultaneously scoring poorly on satisfaction, well-being, and efficiency. More deployments, but developers working nights to hit sprint commitments, skipping design discussions because agents make coding fast enough to skip planning, accumulating cognitive debt from reviewing AI code they don't fully understand. SPACE catches this. DORA doesn't. Running both frameworks gives you the full picture.
...
Standard frameworks weren't designed with agentic coding in mind. These metrics fill the gap.

### % AI-Assisted Code

The proportion of committed code that was agent-generated or agent-assisted.

**Why it matters**: Provides context for everything else. A 5% increase in CFR is a different signal if agents assist 10% of your code versus 80%. Track this as the denominator for all quality metrics.

**How to get it in Vibe**: there is no verified native attribution feature. The live check on 2.25.7 (T8 in [`live-checks.md`](../../docs/mechanics/live-checks.md)) had Vibe author a file and commit it: the commit carries no attribution trailer at all — no co-author line, no generated-with marker. Vibe commits are indistinguishable from human commits unless the team adds attribution itself. That makes this metric a policy choice, not a dashboard:

- an `AGENTS.md` commit-message convention (`Generated with Vibe ...`), which the model follows as instructions but which nothing enforces (PART-AGENTSMD), or
- a `post_tool` hook that logs `bash` commit commands to your own JSONL — the recipe is on [Session Observability](./observability.md) (PART-HOOKS).

For volume rather than attribution, the session store gives you a proxy: count sessions and session metadata per developer per week — `$VIBE_HOME/logs/session/<dir>/meta.json` records `session_id`, `start_time`, `end_time`, `git_branch`, `git_commit`, `username`, `agent_profile`, and `total_messages` (PART-SESSIONS §3.1). Sessions are not the same as code share; treat the count as an adoption signal.
...
Split your Change Failure Rate by code origin: agent-assisted commits versus manually written commits — using whatever tagging convention you adopted above.

If agent-generated CFR is within 2-3 percentage points of manual CFR, your review process is working. If agent-generated CFR is materially higher, review discipline has dropped. If it's lower, the tooling may genuinely be improving code quality in your domain.

### Review Time: AI PRs vs Manual PRs

Compare average review time (open to merge) for agent-generated PRs versus manually written PRs. If agent-assisted PRs are getting merged significantly faster than manual ones, you may have a rubber-stamping problem.

Agent-generated code requires at least as much review scrutiny as manually written code, arguably more, because it can be confidently wrong in non-obvious ways. A 30% faster review cycle for those PRs is a yellow flag worth investigating.

### Developer Code Comprehension

A qualitative, binary signal: during PR review, can the author explain their agent-generated code in their own words — not just what it does, but why it does it that way?
...
Increasing time-to-understand suggests that code is growing more complex or less well-organized over time, regardless of who wrote it. Agent-generated code can inflate this metric by producing syntactically dense implementations that are harder to reason about than simpler, more explicit alternatives.

---

## Measure the Shared Verification Queue

Measure one service and comparable change classes before increasing agent concurrency. Practitioner accounts cited by the source guide (Dex Horthy; Yacine Hmito) describe review remaining the bottleneck despite faster automation, and motivate correcting recurring defects upstream. Neither supplies a universal queue limit.
...
Define pause and resume conditions from a local baseline. Improving the acceptance count by weakening review is not evidence of improvement. Keep the quality policy fixed during a comparison and disclose changes in reviewers, tools or workload. When the policy needs to be enforced rather than hoped for, the verified enforcement stack is deny rules, `strict = true` hooks, and `AGENTS.md` policy — [Production Safety Rules](../security/production-safety.md).
...
| Pass rate on PRs with executable tests as oracle | How often agent-generated code actually passes functional validation | c-CRAB (arXiv 2603.23448, March 2026): 32.1% pass rate on real PRs for the strongest single agent CLI tested. Union of four tools: 41.5%. This is the practical ceiling for current AI code review quality |

The METR -19% figure applies specifically to L1-L2 workflows (a leading AI-native editor and a frontier chat model, on complex existing repos). It is the only rigorous control-group measurement available for this context. Self-reported figures from McKinsey (20-45%), BCG (64%), and GitHub's own studies are not replicated under controlled conditions and should be treated as aspirational estimates, not planning inputs.

### Group 2: Agentic pipeline metrics

These require instrumentation. The Vibe-native path is the session store and a hook logger (both covered in [Session Observability](./observability.md)) plus OTel export for org-level pipelines — `enable_otel`, `otel_endpoint`, `otel_redaction` (PART-CONFIG §1.8). External agent-observability platforms (Langfuse, Arize Phoenix, AWS Bedrock AgentCore) sit alongside, not instead; they are a layer specific to non-deterministic systems.
...
| Validation contract pass rate | Percentage of agent-generated implementations that pass pre-defined behavioral contracts, measured before human review | Factory.ai Missions pattern (vendor-reported): 81 problems detected in a Slack clone from spec alone, generating 34% of implementation work as fix features |
| Agent task completion rate | Tasks the agent completes without human correction, expressed as a percentage | Instrument from the session store or hook logs. No public benchmark exists for tail task duration; second-hand reporting suggests it grows over time as agents take on more complex tasks — treat that as anecdote until you baseline your own |
...
| Cost per completed task | Token spend plus human review time per agent task that reaches a mergeable state | No industry benchmark published yet. In Vibe, per-session statistics live in the session's `meta.json` `stats` field (contents not itemized in the oracle — see [Known Gaps](#known-gaps)), and each headless run can be bounded and observed with `--max-price` / `--max-tokens` (PART-CLI). Mistral token prices are not in the oracle; the estimation method with reader-supplied rates is on [Session Observability](./observability.md) |
| Tokens per feature | Average tokens consumed per merged feature, crossed with Jira or Linear ticket boundaries. Better signal than tokens/request because it accounts for session count variation per feature | No industry benchmark. Aggregate the session store per feature: `meta.json` gives `start_time`/`end_time`, `git_branch`, and `total_messages` per session (PART-SESSIONS §3.1); group sessions by the branch or ticket convention your team uses. The source guide reports a practitioner range of 500K-2M tokens per complete PR on complex codebases (community tooling for another CLI) — treat as anecdote until you baseline your own |
...
For Vibe, the raw material for an agent inventory is real: every session records `session_id`, `parent_session_id`, `child_sessions`, `agent_profile`, `username`, and `git_branch` (PART-SESSIONS §3.1), and a hook logger records every executed tool call with `session_id` in the payload (PART-HOOKS §3.2). Wiring that into a central registry is your pipeline, not a product feature.
...
In Vibe, the CI path for this is programmatic mode: `vibe -p "<prompt>" --output json` emits machine-readable per-run results, and `--max-turns` bounds each run (PART-CLI). Approval-required tool calls are denied, never silently approved, in headless runs — so a CI agent needs explicit permissions (`--agent`, `--auto-approve`, or a per-tool `allowlist` — PART-CLI; PART-CONFIG §1.3) to get real work done (the live denial runs are T4 in [`live-checks.md`](../../docs/mechanics/live-checks.md)).
...
Track this as a median across your user cohorts, and watch for regressions after major feature releases. Agents can accelerate your feature shipping without improving, or even while degrading, the new user experience.
...
| Agentic session volume and mix | Session store: count sessions per developer per week from `meta.json` (`username`, `agent_profile`, `start_time` — PART-SESSIONS §3.1) | Monthly |
| % AI-assisted code | Your commit-tagging convention (see [AI-Specific Metrics](#ai-specific-metrics)); Vibe adds no trailer by default (live-verified on 2.25.7, T8) | Monthly |
| Developer satisfaction | 8-question survey, anonymous | Quarterly |
| PR review time | GitHub Insights or LinearB | Weekly |
| Time-to-value | Analytics tool | Monthly |

**Tooling**: LinearB or Faros.ai for delivery automation, PostHog or Amplitude for product metrics, and the Vibe session store plus your tagging convention for agentic-specific data (capture recipes: [Session Observability](./observability.md)). Verify that your selected delivery tool exposes the current five-metric model and uses DORA's definitions, because existing dashboards may still label all-cause MTTR as a DORA metric.
...
| Lines of code | Bug Escape Rate | LOC measures output volume. With agents, LOC goes up automatically. Bug Escape Rate measures output quality. |
...
### What Vibe gives you to measure with [stable]

Vibe ships no metrics dashboard, and no verified feature computes % AI-assisted code, CFR by code origin, or review-time splits. What it ships are capture surfaces. Everything in the AI-specific sections above gets built on these:

| Surface | What it gives you | Citation |
|---|---|---|
| `/status` | "Display agent statistics" in-session (verbatim description; what it displays is not itemized in the oracle — see [Known Gaps](#known-gaps)) | PART-COMMANDS §2 |
| Per-session stats in `meta.json` | `stats`, `total_messages`, `start_time`/`end_time`, `git_branch`, `git_commit`, `username`, `agent_profile`, `config.active_model` (the pinned model), per session | PART-SESSIONS §3.1, §3.5 |
| Session store as raw material | `$VIBE_HOME/logs/session/<prefix>_<date>_<id8>/` with `messages.jsonl` per session; index at `.session_index.json`; aggregable with `grep`/`jq`. Default `$VIBE_HOME` is `~/.vibe`; per-machine, so org-level aggregation is your collection script or OTel | PART-SESSIONS §3.1; PART-CLI |
| Headless runs in CI | `vibe -p "<prompt>" --output json` (or `streaming`) emits machine-readable per-run results; `--max-turns` bounds the run; approval-required calls are denied, never silently approved | PART-CLI; live-verified on 2.25.7 (T4 in [`live-checks.md`](../../docs/mechanics/live-checks.md)) |
| OTel export | `enable_otel = true` (requires `enable_telemetry`) ships traces to `otel_endpoint` — an OTLP/HTTP base URL where Vibe appends `/v1/traces`; `otel_redaction` = `"default"`/`"none"`/`"strict"`. With an empty endpoint, traces go to Mistral's telemetry endpoint, not yours | PART-CONFIG §1.8 |
| Budget instrumentation | `--max-price DOLLARS` and `--max-tokens N` cap a programmatic (`-p`) run and interrupt it when exceeded; per-model `input_price` / `output_price` / `cached_input_price` floats feed the cap. Budget caps are `-p`-mode flags — there is no verified interactive budget cap | PART-CLI; PART-CONFIG §1.1 |

The how-to-capture recipes for the second, third, and fourth rows — session cataloging, a hook logger, the cost estimation method — live on [Session Observability](./observability.md). Read that page before building anything in this section.

### Third-party delivery and product analytics
...
No third-party tool automatically surfaces the AI-specific metrics described earlier (CFR by code origin, review time comparison, comprehension signals), and neither does Vibe. Those require custom dashboards built on your CI/CD data and the session store, or manual tracking. The session store covers raw volume; the origin split you wire up yourself through your tagging convention.

Avoid tool sprawl. A team with LinearB, Jira, GitHub Insights, and two separate analytics tools will spend more time reconciling numbers than acting on them. Pick one DORA tool, one product analytics tool, and build the agentic layer on the session store.
...
| Cortex.io | Positions itself as an "Engineering Operations Platform" and publishes its own "Engineering Intelligence Platforms: Top 8 Tools" category guide | Cited by the source guide as a data source for PR size and change failure rate figures |
| Jellyfish | Positions itself in 2026 as a "software engineering intelligence platform" explicitly built for AI-integrated organizations | Same note as Cortex.io: a data source in the source guide, now documented as a tool |
...
One AI-specific caution on the input data: if agents changed your cycle-time distribution in the last quarter, the "recent change to process" rule applies. Feed the forecaster post-adoption cycle times only, or the distribution mixes two different delivery systems.
...
Once DORA is stable and automated, add the product metrics (time-to-value, feature CSAT) and the AI-specific signals (agentic session volume, CFR by code origin). In Vibe terms, this is when the session store and the commit-tagging convention start earning their keep (PART-SESSIONS §3.1; see [Session Observability](./observability.md)).
...
The [probabilistic forecasting](#probabilistic-delivery-forecasting) tools covered earlier on this page give you the honest range behind each scenario: "80% confidence between these two dates" is a more defensible number to bring into the room than a single point estimate, provided your delivery system is stable enough for the forecast to mean something. The [AI-generated board narratives](#ai-generated-board-narratives) from tools like LinearB and Jellyfish reduce the manual effort of translating sprint data into a written summary a non-technical board member can read.
...
- **`/status` contents** — the command's description is "Display agent statistics" (PART-COMMANDS §2), but the oracle does not itemize what it displays. No verified in-CLI command reports token counts or spend. Do not promise a specific readout before checking your own install.
- **`meta.json` `stats` subfields** — the field exists and is written per session (PART-SESSIONS §3.1), but its contents are not itemized in the oracle. Queries against its subfields are unverified; do not build a pipeline on them as fact.
- **`messages.jsonl` per-message schema** — the line-per-message shape is verified; message-internal fields are not. The hook logger is the verified substitute for message-level queries ([Session Observability](./observability.md)).
- **OTel span and attribute field names** — only the config keys (`enable_otel`, `otel_endpoint`, `otel_redaction` — PART-CONFIG §1.8) are verified. Any dashboard query keyed to specific span fields is unverified until you inspect one export from your own collector.
- **No native attribution metric** — Vibe adds no commit trailer (live-verified on 2.25.7, T8), and no verified feature computes % AI-assisted code. That split is a team convention built on hooks and policy, not a product feature.
- **Token prices** — Mistral token prices are not in the oracle; the cost-estimation method deliberately uses reader-supplied rates ([Session Observability](./observability.md)).
- **External study figures** — METR, DeputyDev, c-CRAB, Strata, CSA/Zenity, Digital Applied, and Uplevel numbers are reproduced from the source guide's citations as dated external evidence. This guide's oracle did not verify them.
- **Vibe Code Web and desktop** — no team-level metrics or dashboard surface on Vibe Code Web or the desktop app is verified here; PART-WEB quota and session mechanics are docs-only on those points.
- **Unified-harness session store** — whether `~/.vibe/logs/session/unified/` shares the `meta.json`/`messages.jsonl` schema is unresolved in the oracle; do not write tooling against it.

---

## See Also

- [Session Observability](./observability.md) — the capture mechanics this page assumes: session store fields, the hook logger recipe, OTel configuration, and the cost-estimation method
- [Architecture and Internals](../core/architecture.md) — where the session store and the agent loop sit, and what the loop records
- [Production Safety Rules](../security/production-safety.md) — the enforcement stack (deny rules, strict hooks, `AGENTS.md` policy) behind any quality policy you fix while measuring the verification queue
- [Settings Reference](../core/settings-reference.md) — every `config.toml` key used here, including `[session_logging]` and the OTel block
- The mechanics oracle: [`verified-mechanics.md`](../../docs/mechanics/verified-mechanics.md); live checks: [`live-checks.md`](../../docs/mechanics/live-checks.md)
